...
const TaskPage = () => {
  ...
  console.log("Is Projects Pending:", isProjectsPending);
  console.log("Is Projects Error:", isProjectsError);

  //current project map
  const currentProject = projectsData?.projects.find(p => p._id === projectId);

  const updateTaskProject = (newProjectId: string) => {
    console.log(`Updating task's project to: ${newProjectId}`);
    ...
    // After successful update, route to task since project is changed and invalidate task query to refetch with new project data
        navigate(`/workspace/${workspaceId}/project/${newProjectId}/task/${taskId}`);
          ...
          <div className="bg-white text-gray-900 min-h-screen p-4 sm:p-6 lg:p-8">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <main className="lg:col-span-2 space-y-6">
                {/* Header */}
                <div>
                  <span className="text-sm text-gray-500 hover:text-blue-600 cursor-pointer">Projects / Tiqueon / {data?.taskCode}</span>
                ...
                <Card>
                  <CardHeader>
                    <CardTitle>Attachments ({data?.files.length})</CardTitle>
                  </CardHeader>
                  <CardContent className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                    {data?.files.map((att) => (
                      <div key={att._id} className="border rounded-lg p-2 text-center text-sm space-y-1">
                        <div className="w-full h-16 bg-gray-100 rounded flex items-center justify-center">
                          <Paperclip className="h-8 w-8 text-gray-400" />
                        </div>
                        <p className="truncate">{att.filename}</p>
                        <Button variant="link" size="sm" className="h-auto p-0" onClick={() => handleDownload(att.fileUrl, att.filename)}
                        >Download</Button>
                      </div>
                    ))}
                ...
                <Card>
                  <CardContent className="p-4 space-y-4">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <Select value={projectId} onValueChange={(projectId) => {
                        const newproject = projectsData?.projects.find(p => p._id === projectId);
                        if (newproject) {
                          updateTaskProject(projectId); // Implement this function to update the task's project
                        }
                      }}>
                        <SelectTrigger className="w-[180px]">
                          <div className="flex items-center space-x-2">
                            <Avatar className="h-6 w-6">
                              <AvatarImage src={currentProject?.emoji} alt={currentProject?.name} />
                        ...
                        <SelectContent>
                          {projectsData?.projects.map(project => (
                            <SelectItem key={project._id} value={project._id}>
                              <div className="flex items-center space-x-2">
                                <Avatar className="h-6 w-6">
                                  <AvatarImage src={project?.emoji} alt={project.name} />
                ...
                <Card>
                  ...
                  <CardContent className="space-y-3 text-sm">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span>{formatDistanceToNow(data.updatedAt, { addSuffix: true })}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-gray-600">Due Date</span>
                      <span>{format(data.dueDate, "MMM d, yyyy")}</span>
                    </div>
                  </CardContent>
                </Card>
